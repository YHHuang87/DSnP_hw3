...
CmdParser::openDofile(const string& dof)
{
   ...
   if(_FileIsOpen == 1 && _dofileStack.size() < 1024){
     //cout << _dofile->is_open() << endl;
     _dofileStack.push(_dofile);
     //cout << _dofileStack.top()->is_open() << endl;
     //delete _dofile;
     _FileIsOpen = 0;
   }
   else if(_FileIsOpen == 1 && _dofileStack.size() == 1024){ 
     //delete _dofile;
     return true;
   }
   if(_FileIsOpen == 0) _dofile = new ifstream(dof.c_str());
   if(_dofile->is_open() == 0){
     _dofile->close();
...
{
   // TODO...
   map <const string, CmdExec*> :: const_iterator iter  = _cmdMap.begin();
...
CmdParser::parseCmd(string& option)
{
   ...
   size_t t;
   // TODO...
   t = myStrGetTok(str,tok);
   map <const string, CmdExec*> :: iterator iter;
   for(map <const string, CmdExec*> :: iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
     string cmdfml = iter->first;
     cmdfml.append(iter->second->getOptCmd());
     int ans = myStrNCmp(cmdfml,tok,iter->first.length());
     //cout << ans << endl;
     if(ans == 0){
       if(str.length()>tok.length()){
         //cout << str.length() << ' ' << tok.length() << endl;
         option = str.substr(t);
         //cout << option << endl;
       }
       return getCmd(iter->first);
     }
   }
   cerr << "Illegal command!! \"(" << tok << ")\"" << endl;
...
CmdParser::listCmd(const string& str)
{
   ...
   if(blanks == 1 || str == "\0"){
     ...
     if(token.size() == str.size()){
       ...
       for(map <const string, CmdExec*> :: iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
        int size = 0;
        if(token.size() > iter->first.size()){
          size = iter->first.size();
          if(myStrNCmp(token, iter->first, size) == 0){
            match_num ++;
          }
        }
        else{
          size = token.size();
          if(myStrNCmp(iter->first, token, size) == 0){
            match_num ++;
            
          //cout << "ggood" << endl;
          }
        }
        //cout << token.size() << ' ' << iter->first.size() << ' ' << iter->first << ' ' << size << endl;
        //cout << myStrNCmp(iter->first, token, size);
       }
       if(match_num > 1){
         int count = 0;
         cout << endl;
         ...
         for(map <const string, CmdExec*> :: iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
          ...
          if(token.size() > iter->first.size()){
            size = iter->first.size();
            if(myStrNCmp(token, iter->first, size) == 0){
              //cout << iter->first.size() << ' ' << token.size() << ' ' << pos << endl;
              if(iter->first.size() > token.size()){
                string s = iter->first.substr(token.size());
                for(int i = 0; i < s.length(); i++){
                  insertChar(s[i]);
                }
              }
              string s2 = iter->second->getOptCmd().substr(token.size()-iter->first.size());
            ...
            if(myStrNCmp(iter->first, token, size) == 0){
              ...
              string s2 = iter->second->getOptCmd();
              for(int i = 0; i < s2.length(); i++){
                insertChar(s2[i]);
              }
              insertChar(' ');
          ...
          if(myStrNCmp(iter->first,token,size) == 0){
            ...
            if(_tabPressCount == 1){
              ...
              //cout << str[str.size()-1] << endl;
                ...
                //cout << "b" << endl;
                string prefix = str.substr(token.size()+1);
                //cout << prefix << endl;
                ...
                if(files.size() == 0){
                  mybeep();
                  min_size = prefix.size();
                }
                bool jump = 0;
                for(int i = prefix.size(); i < min_size; i++){
                  char cmpr = (files[0])[i];
                  //cerr << cmpr << endl;
                ...
                //cout << same_or_not << endl;
                if(same_or_not == 0 && files.size() > 1){
                  cout << endl;
                  for(int i = 0; i < files.size(); i++){
                  ...
                  for(int i = prefix.size(); i < prefix.size() + same_or_not; i++){
...
CmdParser::getCmd(string cmd)
{
   ...
   for(iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++){
     //cout << "fuck" << endl;
     int ans = myStrNCmp(iter->first, cmd, iter->first.length());
     //cout << ans << endl;
     if(ans == 0){
       //cout << "good!!" << endl;
       return iter->second;
