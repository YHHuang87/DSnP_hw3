...
DBAppendCmd::exec(const string& option)
{
   ...
   string dummy;
   int num;
   if(lexOptions(option, k_v, 2) == 1){
     if(isValidVarName(k_v[0])){
        if(myStr2Int(k_v[1],num)){
          DBJsonElem temp(k_v[0],num);
          if(dbjson.add(temp)) return CMD_EXEC_DONE;
          else {
            cerr << "Error: Element with key \"" << k_v[0] << "\" already exists!!" << endl;
            return CMD_EXEC_ERROR;
          }
        }
        else return errorOption(CMD_OPT_ILLEGAL, k_v[1]);
     }
     else return errorOption(CMD_OPT_ILLEGAL, k_v[0]);
   }
   return CMD_EXEC_ERROR;
}
...
{  
   ...
   if(option == "\0"){
     ...
     if(lexOptions(option,temp,1)){
        for(int i = 0; i < dbjson.size(); i++){
                if(dbjson[i].key() == temp[0]){
                cout << dbjson[i] << endl;
                return CMD_EXEC_DONE;
                }
        }
        cerr << "Error: No JSON element with key \"" << temp[0] << "\" is found." << endl;
        return CMD_EXEC_ERROR;
     }
     else{
        return CMD_EXEC_ERROR;
